...
class INET_API r438StreamRedundancyConfigurator : public NetworkConfiguratorBase
{
  protected:
    ...
    class INET_API Topology : public NetworkConfiguratorBase::Topology {
      protected:
        virtual Node *createNode(cModule *module) override { return new r438StreamRedundancyConfigurator::Node(module); }
    };
...
//    std::map<std::pair<std::string, std::string>, int> nextVlanIds; // maps network node name and destination node name to next available VLAN ID
    std::map<std::pair<std::string, int>, int> nextVlanIds; // maps network node name and stream_id from streamName to next available VLAN ID
