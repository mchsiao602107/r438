...
void r438StreamRedundancyConfigurator::initialize(int stage)
{
...
void r438StreamRedundancyConfigurator::handleParameterChange(const char *name)
{
    if (name != nullptr) {
...
void r438StreamRedundancyConfigurator::clearConfiguration()
{
    streams.clear();
    nextVlanIds.clear();
...
void r438StreamRedundancyConfigurator::computeConfiguration()
{
...
void r438StreamRedundancyConfigurator::computeStreams()
{
...
void r438StreamRedundancyConfigurator::computeStreamSendersAndReceivers(cValueMap *streamConfiguration)
{
...
//                auto jt = nextVlanIds.emplace(std::pair<std::string, std::string>{networkNodeName, destinationAddress}, 0);
                ...
                if (streamName.substr(0, 3) == "tsn") {
                    sscanf(streamName.c_str(), "tsn-%d", &stream_id);
                }
                else {
                    sscanf(streamName.c_str(), "avb-%d", &stream_id);
                }
                auto jt = nextVlanIds.emplace(std::pair<std::string, int>{networkNodeName, stream_id}, 2 * stream_id);
...
void r438StreamRedundancyConfigurator::computeStreamPolicyConfigurations(cValueMap *streamConfiguration)
{
    std::string sourceNetworkNodeName = streamConfiguration->get("source");
    std::string destinationAddress = streamConfiguration->containsKey("destinationAddress") ? streamConfiguration->get("destinationAddress") : streamConfiguration->get("destination");
    std::string streamName = streamConfiguration->get("name").stringValue();
...
void r438StreamRedundancyConfigurator::configureStreams()
{
    for (int i = 0; i < topology->getNumNodes(); i++) {
        auto node = (Node *)topology->getNode(i);
...
void r438StreamRedundancyConfigurator::configureStreams(Node *node)
{
    auto networkNode = node->module;
...
std::vector<std::string> r438StreamRedundancyConfigurator::getStreamNames()
{
...
std::vector<std::vector<std::string>> r438StreamRedundancyConfigurator::getPathFragments(const char *stream)
{
    for (int i = 0; i < configuration->size(); i++) {
        cValueMap *streamConfiguration = check_and_cast<cValueMap *>(configuration->get(i).objectValue());
